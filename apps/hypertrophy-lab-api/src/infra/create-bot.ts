...
import { claimLinkService } from '../services/integrations.telegram.service.js';
import { telegramAction } from '../services/schedule.telegram.service.js';
...
export async function createBot(env: ApiEnv) {
  ...
  bot.start(async (ctx) => {
    ...
    try {
      if (!ctx.chat.id) throw new Error('Chat id missing.');
      res = await claimLinkService({ code, chatId: String(ctx.chat.id) });
    } catch (err) {
    ...
    if (!res.ok) {
      return ctx.reply('Link failed: ' + (res?.error || 'error'));
    }
  ...
  bot.on('callback_query', async (ctx) => {
    ...
    try {
      const res: TelegramActionResponse = await telegramAction({
        action: action as 't' | 's',
        logId,
      ...
      await ctx.answerCbQuery(
        res.status === 'taken' ? 'Marked taken' : 'Marked skipped',
        {
          show_alert: true,
        },
      );
